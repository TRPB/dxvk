...
- `dxvk.particleSkipSampleShading = false` *(default, disabled)* - Same idea as `transparentSkipSampleShading` but applied to soft-alpha particles: standard or premultiplied alpha blend, multisampled pass with a depth-stencil attachment, alpha-test off. The DS-attached + MSAA gate excludes UI/text; the alpha-test exclusion (d3d9 only) keeps alpha-blended foliage edges like WoW 3.3.5a's grass on per-sample shading. Heuristic; off by default. Turn on if smoke effects tank your framerate.
...
- `dxvk.particleSkipAlphaTested = false` *(default, disabled)* - Opt-in extension to `particleSkipSampleShading` (and the other particle options): when `true`, the particle classifier ALSO includes pipelines that have D3D9 alpha-test enabled. Default `false` keeps alpha-tested foliage (e.g. WoW 3.3.5a grass) on per-sample shading. Set to `true` for older D3D9 games whose smoke uses alpha-test as a "discard fully transparent" optimization (e.g. Freelancer 2003) — otherwise the particle path won't catch their smoke. No effect on D3D11.

  Quick reference: all four particle options follow opt-in semantics — `true` enables more optimization, `false` disables. Setting all four (`particleSkipSampleShading`, `particleShadingRate != 1x1`, `particleMipBias != 0.0`, `particleSkipAlphaTested`) to "on" applies the maximum optimization to the broadest set of pipelines.
